...
let lastBenignIndex = -1;
let lastMalignantIndex = -1;
...
function loadBenignSample() {
    ...
    do {
        randomIndex = Math.floor(Math.random() * benignSamples.length);
    } while (randomIndex === lastBenignIndex);

    lastBenignIndex = randomIndex;

    fillForm(benignSamples[randomIndex]);
}
...
function loadMalignantSample() {
    ...
    do {
        randomIndex = Math.floor(Math.random() * malignantSamples.length);
    } while (randomIndex === lastMalignantIndex);

    lastMalignantIndex = randomIndex;

    fillForm(malignantSamples[randomIndex]);
}
